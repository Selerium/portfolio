      ...
      <div className="w-11/12 h-full flex flex-col gap-10">
        <div className="max-w-3xl">
          ...
          <p className="mt-4 text-lg font-light max-w-2xl">
            A closer look at the products, systems, and engineering projects behind the work — ordered to surface the most relevant full-stack, backend, and infrastructure experience first.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-8 items-stretch">
          {projects.map((project, index) => (
            <article key={project.slug} className="projectList relative border rounded-lg min-h-80 min-w-72 w-80 grow flex basis-1/4 justify-center items-start overflow-clip group">
              <img
                className="absolute w-full h-full object-cover project-images object-top z-10 opacity-40 transition-transform duration-500 group-hover:scale-105"
                src={basePath + project.image}
                alt={project.title + " project preview"}
              />
              <div className="absolute top-3 left-3 z-20 rounded-full bg-white/90 px-3 py-1 text-xs font-semibold uppercase tracking-widest">
                {index < 4 ? "featured" : "project"}
              </div>
              ...
              <div className="projectOverlay transition-all text-black p-4 absolute rounded-lg z-30 border-white bg-white h-full w-full flex flex-col gap-4 items-start overflow-y-auto">
                ...
                <div className="flex flex-wrap gap-2 w-full">
                  {project.tags.map((tag) => (
                    <span key={tag} className="p-2 min-w-fit bg-gray-300 rounded-lg font-semibold">{tag}</span>
                  ))}
                </div>
                <Link href={"/projects/" + project.slug} className="relative mt-auto transition-all cursor-pointer text-center w-full p-2 rounded-lg bg-primary text-white uppercase tracking-widest hover:bg-black focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
                  VIEW CASE STUDY -&gt;
                </Link>
              </div>
            </article>
