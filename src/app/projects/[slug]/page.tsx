  ...
  return (
    <main className="w-full min-h-dvh py-16 lg:py-24 flex justify-center">
      <article className="w-11/12 max-w-6xl">
        <Link
          href="/projects"
          className="link inline-flex mb-10 text-sm uppercase tracking-[0.25em] font-semibold"
        >
        ...
        <header className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-end">
          <div>
            <p className="uppercase tracking-[0.3em] text-sm font-semibold text-primary">
              case study
            </p>
        ...
        <div className="mt-16 grid lg:grid-cols-[1fr_280px] gap-12">
          <div className="space-y-14">
            <section>
              <h2 className={primary.className + " text-3xl font-semibold tracking-tight"}>
                overview
              </h2>
            ...
            <section>
              ...
              <ul className="mt-5 space-y-3 list-disc ml-5">
                {project.highlights.map((highlight) => (
                  <li key={highlight} className="font-light leading-relaxed">
                    {highlight}
                  </li>
                ))}
              </ul>
            ...
            {project.mediaNote && (
              <section className="rounded-xl border border-dashed border-primary p-6">
                <h2 className={primary.className + " text-3xl font-semibold tracking-tight"}>
                  next-level case study content
                </h2>
                <p className={secondary.className + " mt-4 font-light leading-relaxed"}>
                  {project.mediaNote}
                </p>
        ...
        <nav className="mt-16 pt-8 border-t border-primary flex flex-col sm:flex-row gap-4 justify-between">
          {previous ? (
            <Link href={"/projects/" + previous.slug} className="link">
              &larr; {previous.title}
            </Link>
          ) : (
            <span />
          ...
          {next ? (
            <Link href={"/projects/" + next.slug} className="link sm:text-right">
              {next.title} &rarr;
            </Link>
          ) : (
            <span />
