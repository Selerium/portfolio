...
export default function Home() {
  ...
  return (
    <div className="h-dvh py-4 box-border w-full flex flex-col justify-end items-center overflow-x-clip relative z-0">
        ...
        {imageSrcs.slice(0, 3).map((img, index) => (
          <img
            ...
            className={`transition-all duration-300 absolute rounded-lg object-cover ${
              index == activeElement
                ? "z-30 h-5/6 sample-shadow"
                : "z-10 h-3/6 opacity-25"
            }
      ...
      <div className="w-11/12 h-2/12 flex justify-between relative items-center">
        ...
        <div className="flex flex-col justify-center items-center gap-4">
          ...
          <Link
            href="/connect"
            className={`link cursor-pointer text-xl tracking-widest font-semibold ${primary.className} text-center uppercase py-2 pl-7 px-4 bg-primary border border-white w-fit rounded-lg`}
          >
            get a quote
          </Link>
        </div>
