  ...
  function Sidebar() {
    return (
      <div className={`${showSidebar ? 'opacity-100 z-50' : 'opacity-0 -z-50 translate-y-full'} absolute transition-all top-0 w-dvw h-dvh flex flex-col gap-12 justify-center items-center bg-black`}>
        ...
        {pages.map(page => <p className={`cursor-pointer delayed-text text-3xl ${primary.className} ${showSidebar ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-full'} transition-all uppercase tracking-widest font-semibold`} key={`${page}`} onClick={toggleSidebar}>{ page }</p>)}
      </div>
    )
  }
