...
export default function Connect() {
    async function submitForm(event: FormEvent<HTMLFormElement>) {
        ...
        formInfo.forEach((value, key) => console.log(key + ': ' + value));
    }

    return (
        <div id="connect" className={`h-dvh py-4 box-border w-full flex justify-center items-center relative z-0`}>
                    ...
                    <div className="flex flex-wrap gap-4 w-full justify-center">
                        ...
                        <div className="grow w-full min-w-72 flex flex-col gap-2">
                            ...
                            <select name="enquiryType" className={`${secondary.className} font-light text-md tracking-tight lowercase bg-white rounded-lg p-2 text-black`}>
                                ...
                                <option className="p-2 bg-white text-black" value={0}>employment offer</option>
                                <option className="p-2 bg-white text-black" value={1}>get a quote for dev job</option>
                                <option className="p-2 bg-white text-black" value={2}>just wanna say hi :)</option>
                            </select>
                        </div>
                        <div className="grow w-full min-w-72 flex flex-col gap-2">
                            <label className={`${secondary.className} font-light text-md tracking-tight`}>leave your thoughts</label>
                            <textarea name="description" rows={5} className={`${secondary.className} resize-none font-light text-md tracking-tight lowercase bg-white rounded-lg p-2 text-black`}>
                            </textarea>
                        </div>
                        <button type="submit" className={`uppercase ${primary.className} cursor-pointer tracking-widest font-semibold pl-6 m-4 pr-4 py-2 rounded-lg bg-primary border border-white`}>submit</button>
                    </div>
                </form>
            </div>
        </div>
